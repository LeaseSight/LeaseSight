          ...
          {items.map((item) => (
            <div 
              ...
              className={`group p-4 rounded-2xl border transition-all cursor-pointer ${
                selectedDoc === item.file_name && annotations[0]?.page === targetPage ? 'border-blue-500 bg-blue-50/30' : 'border-slate-200 bg-white hover:border-blue-300'
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest ${
                    item.confidence > 0.8 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                  }`}>
                    {item.category}
                  </div>
